...
class NovaAPIServer {
    ...
    func start() {
        do {
            ...
            listener?.stateUpdateHandler = { if case .ready = $0 { print("NovaAPI [NewsTV]: port \(self.port)") } }
            listener?.start(queue: .main)
        } catch { print("NovaAPI [NewsTV]: failed — \(error)") }
    }
